...
def query_influx(start: datetime, stop: datetime) -> dict:
    ...
    queries = {
        ...
        "db_prod": f'from(bucket: "{INFLUX_BUCKET}") |> range(start: {s_str}, stop: {p_str}) |> filter(fn: (r) => r._measurement == "status_check_oracleDataBase") |> filter(fn: (r) => r._field == "status_value") |> filter(fn: (r) => r.ambiente == "PRODUCCION" or r.ambiente == "PROD") |> filter(fn: (r) => r._value == 0)',
